...
//  override def run: IO[Unit] = (ioa, iob).parMapN(_ + " " + _).customDebug.void
  ...
  val iob: IO[String] =
    IO("world").customDebug *> IO.raiseError(new RuntimeException("lolololol"))

  def myParMapNNaive[A, B, C](ia: IO[A], ib: IO[B])(f: (A, B) => C): IO[C] =
    for {
      iaFiber <- ia.start
      ibFiber <- ib.start
  ...
  def myParMapN[A, B, C](ia: IO[A], ib: IO[B])(f: (A, B) => C): IO[C] = {
    def handleOutcome[T](outcome: OutcomeIO[T]): IO[T] = outcome match {
      ...
      case Canceled() =>
        IO.raiseError(new RuntimeException("ooops...cancelled"))
    }
  ...
  /** Races the effect a against a set timeout. Return type is a bit wonky and
    * should maybe just fail the IO with a custom exception but this works-ish
    */
  def withTimeout[A](a: IO[A], timeout: FiniteDuration): IO[Either[String, A]] =
    IO.race(IO.sleep(timeout), a)
